# from deep_sort.deep_sort.nn_matching import nn_matching
...
class deepsort_rbc():
    ...
    def run_deep_sort(self, frame, out_scores, out_boxes):
        ...
        crops = self.pre_process(frame, detections)

        if len(crops) == 0:
            self.tracker.predict()
            return self.tracker, []

        processed_crops = crops.cuda()
